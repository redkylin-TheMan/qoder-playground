...
function stopAudioPlay() {
    ...
    WebVideoCtrl.I_StopAudioPlay().then(() => {
        showOPInfo(szDeviceIdentify + " 音频暂停成功！");
    }).catch(() => {
        showOPInfo(szDeviceIdentify + " 音频暂停失败！");
    });
}
...
function startAudioPlay() {
    ...
    var szDeviceIdentify = $("#ip").val(),
        iAudioChannel = parseInt($("#AudioChannel").val(), 10)
    ...
    WebVideoCtrl.I_AudioPlay(szDeviceIdentify, iAudioChannel, callBackFun).then(() => {
        showOPInfo(szDeviceIdentify + " 音频监听开始成功！");
    }).catch(() => {
        showOPInfo(szDeviceIdentify + " 音频监听失败！");
    });
}

// 获取音频通道
function clickGetAudioInChannel() {
    var szDeviceIdentify = $("#ip").val();
    ...
    WebVideoCtrl.I_GetAudioInChannel(szDeviceIdentify, {
        success: function (xmlDoc) {
            var oAudioChannels = $(xmlDoc).find("AudioChannel"),
                oSel = $("#AudioChannel").empty();
            $.each(oAudioChannels, function () {
                var id = $(this).find("id").eq(0).text();

                oSel.append("<option value='" + id + "'>" + id + "</option>");
            });
            showOPInfo(szDeviceIdentify + " 获取音频通道成功！");
        },
        error: function (oError) {
            showOPInfo(szDeviceIdentify + " 获取音频通道失败！", oError.errorCode, oError.errorMsg);
        }
    });
}